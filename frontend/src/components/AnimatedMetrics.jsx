...
// Custom Recharts Tooltip styled to match design tokens
function CustomChartTooltip({ active, payload, label }) {
  if (active && payload && payload.length) {
    return (
      <div className="bg-[#FAF9F6] border border-slate-300 p-2.5 shadow-md font-mono text-xs text-pharma-navy space-y-1 rounded-none">
        <p className="font-bold text-slate-800 font-display">{label || payload[0].name}</p>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between border-b border-slate-200 pb-2">
        <div className="flex items-center space-x-2">
          <Activity className="w-4 h-4 text-pharma-navy" />
        ...
        <span className="text-[11px] font-mono text-slate-500 flex items-center">
          <TrendingUp className="w-3 h-3 text-seal-emerald mr-1" /> Real-time backend feed
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="doc-panel bg-white/80 p-4 border border-slate-300 shadow-xs flex items-center space-x-3.5">
          ...
          <div>
            <span className="text-xs font-semibold text-slate-500 block">Total batches created</span>
            <span className="text-xl font-extrabold text-pharma-navy font-mono">
        ...
        {/* Metric 2 */}
        <div className="doc-panel bg-white/80 p-4 border border-slate-300 shadow-xs flex items-center space-x-3.5">
          <div className="p-3 bg-sky-50 text-sky-900 border border-sky-300 shrink-0">
            <ArrowRightLeft className="w-5 h-5" />
          </div>
          <div>
            <span className="text-xs font-semibold text-slate-500 block">Custody transfers logged</span>
            <span className="text-xl font-extrabold text-pharma-navy font-mono">
        ...
        {/* Metric 3 */}
        <div className="doc-panel bg-white/80 p-4 border border-slate-300 shadow-xs flex items-center space-x-3.5">
          <div className="p-3 bg-emerald-50 text-seal-emerald border border-emerald-300 shrink-0">
            <ShieldCheck className="w-5 h-5" />
          </div>
          <div>
            <span className="text-xs font-semibold text-slate-500 block">Verifications run</span>
            <span className="text-xl font-extrabold text-pharma-navy font-mono">
        ...
        {/* Metric 4 */}
        <div className="doc-panel bg-white/80 p-4 border border-slate-300 shadow-xs flex items-center space-x-3.5">
          <div className="p-3 bg-rose-50 text-quarantine-crimson border border-rose-300 shrink-0">
            <ShieldAlert className="w-5 h-5" />
          </div>
          <div>
            <span className="text-xs font-semibold text-slate-500 block">Alerts & recalls</span>
            <span className="text-xl font-extrabold text-quarantine-crimson font-mono">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="h-52 w-full pt-1">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={metrics.dailyTrends} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Bar dataKey="verifications" name="Verifications" fill="#134E35" radius={[2, 2, 0, 0]} />
                <Bar dataKey="transfers" name="Custody transfers" fill="#0D192B" radius={[2, 2, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>

          <div className="flex items-center justify-center space-x-6 text-xs pt-1 border-t border-slate-200">
            <div className="flex items-center space-x-1.5 font-mono">
              <span className="w-2.5 h-2.5 bg-seal-emerald inline-block" />
              <span className="text-slate-600 text-[11px]">Verifications</span>
            </div>
            <div className="flex items-center space-x-1.5 font-mono">
              <span className="w-2.5 h-2.5 bg-pharma-navy inline-block" />
              <span className="text-slate-600 text-[11px]">Custody transfers</span>
            </div>
          </div>
        </div>
        ...
        <div className="doc-panel bg-white/80 p-4 border border-slate-300 space-y-3 flex flex-col justify-between">
          <div className="flex items-center justify-between border-b border-slate-200 pb-2">
            ...
            <span className="text-[10px] font-mono text-slate-500">Tiered status</span>
          </div>
          ...
          <div className="h-44 w-full relative flex items-center justify-center">
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
              <span className="text-xs font-bold text-pharma-navy font-mono">
          ...
          <div className="space-y-1.5 pt-2 border-t border-slate-200 text-xs font-mono">
            {metrics.verdictBreakdown.map((item, idx) => (
              <div key={idx} className="flex items-center justify-between text-[11px]">
                ...
                <span className="font-bold text-pharma-navy">{item.value}</span>
              </div>
            ))}
          </div>
        </div>
