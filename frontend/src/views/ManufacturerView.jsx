...
import { Factory, Upload, CheckCircle2, ExternalLink, Download, RefreshCw, ChevronDown, ChevronUp } from 'lucide-react';
...
export default function ManufacturerView() {
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      toast.success('Reference packaging photograph attached.');
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!selectedFile) {
      toast.error('Please attach a reference packaging photograph.');
      return;
    }
    ...
    try {
      ...
      toast.success(`Batch #${result.batchId} successfully registered!`);
    } catch (err) {
      console.error(err);
    ...
    <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-start space-x-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-600 mt-0.5 max-w-2xl leading-relaxed">
              Create a verifiable digital origin record and reference packaging photograph for supply chain tracking.
            </p>
          </div>
        </div>
      </div>
      ...
      {!createdBatch ? (
        <form onSubmit={handleSubmit} className="bg-white rounded p-5 sm:p-6 border border-slate-300 doc-panel space-y-5">
          ...
          {/* Reference Packaging Photograph Stage */}
          <div className="space-y-1">
            <label className="block text-xs font-semibold text-slate-700">
              Reference packaging photograph *
            </label>
            <div className="border border-slate-300 rounded p-4 text-center bg-slate-50">
              {filePreview ? (
                <div className="space-y-2">
                  <div className="relative inline-block border border-slate-300 rounded overflow-hidden bg-slate-900 max-h-48">
                    <img src={filePreview} alt="Reference preview" className="max-h-44 mx-auto object-contain" />
                  </div>
                  <p className="text-xs text-slate-600">{selectedFile?.name}</p>
                  <button
                    type="button"
                ...
                <label className="cursor-pointer flex flex-col items-center justify-center space-y-2 py-4">
                  <Upload className="w-6 h-6 text-clinical-800" />
                  <span className="text-xs font-semibold text-clinical-900">Attach reference packaging photograph</span>
                  <span className="text-[11px] text-slate-500">Center packaging under clear lab light (PNG, JPG up to 10MB)</span>
        ...
        <div className="bg-white rounded p-6 border border-slate-300 doc-panel space-y-5">
          <div className="flex items-center space-x-3 text-genuine-600 border-b border-slate-200 pb-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">Digital origin record issued for supply chain tracking</p>
            </div>
          </div>
              ...
              <div className="bg-slate-50 p-3 rounded border border-slate-200 space-y-2">
                ...
                {showTechDetails && (
                  <div className="pt-2 border-t border-slate-200 space-y-1.5 text-[11px]">
                    <div>
                      <span className="text-slate-500 block">IPFS CID Hash:</span>
                      <a
                        href={createdBatch.ipfsUrl}
                        target="_blank"
                        rel="noreferrer"
                        className="font-mono text-clinical-800 hover:underline flex items-center truncate"
                      >
